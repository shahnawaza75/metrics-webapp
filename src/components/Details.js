  ...
  return (
    <div className='flex bg-pink-100 flex-col text-white'>
     <div className='flex items-center justify-center space-x-4 h-32'>
            <img src={allGames[id].image} alt='image' className='w-1/3'/>
            <div>
            <h1 className='font-extrabold text-2xl'>{allGames[id].name}</h1>
            <p className='font-semibold text-lg'>$ {allGames[id].current_price}</p>
            </div>
         </div>
    ...
    <li className='flex items-center justify-between bg-pink-300 p-3 h-14 text-start'>
      <span>symbol:</span> <span>{allGames[id].symbol}</span>
      </li>
    <li className='flex items-center justify-between bg-pink-300 p-3 h-14 text-start'>
      <span>current price:</span> <span>$ {allGames[id].current_price}</span>
    </li>
    <li className='flex items-center justify-between bg-pink-400 p-3 h-14 text-start'>
      <span>rank:</span> <span>{allGames[id].market_cap_rank}</span>
    </li>
    <li className='flex items-center justify-between bg-pink-300 p-3 h-14 text-start'>
      <span>ath:</span> <span>{allGames[id].ath}</span>
      </li>
    <li className='flex items-center justify-between bg-pink-400 p-3 h-14 text-start'>
      <span>market cap:</span> <span>{allGames[id].market_cap}</span>
    </li> 
    <li className='flex items-center justify-between bg-pink-300 p-3 h-14 text-start'>
      <span>total supply:</span> <span>{allGames[id].total_supply}</span>
    </li>
    <li className='flex items-center justify-between bg-pink-400 p-3 h-14 text-start'>
      <span>total volume:</span> <span>{allGames[id].total_volume}</span>
    </li>
    </ul>
    )}
    </div>
